...
\begin{figure}[h]
    ...
    \node[box] (1_1) [above=of COL2_MID]{sensory data};
    \node[box] (3_1) [below=of COL2_MID] {entorhinal cortex};
    \node[box] (4_1) [below=of 3_1] {hippocampus};

    
    \draw[->] (1_1.south) to (2_1.north);
    \draw[->] (1_1.south) to (2_2.north);

    \draw[->] (2_1.south) to ([xshift=-0.75cm]3_1.north);
    \draw[<-] ([xshift=0.5cm]2_1.south) to ([xshift=-0.25cm]3_1.north);
    \draw[<-] ([xshift=-0.5cm]2_2.south) to ([xshift=0.25cm]3_1.north);
    \draw[->] (2_2.south) to ([xshift=0.75cm]3_1.north);

    \draw[->] ([xshift=-0.25cm]3_1.south) to ([xshift=-0.25cm]4_1.north);
    \draw[<-] ([xshift=0.25cm]3_1.south) to ([xshift=0.25cm]4_1.north);
    \end{tikzpicture}
    \label{MechofExpMem}
    \caption{Mechanism of Explicit Memory\protect\footnotemark}
\end{figure}
\footnotetext{from~\cite[p. 207]{kolbIntroductionBrainBehavior2019}, slightly modified}
