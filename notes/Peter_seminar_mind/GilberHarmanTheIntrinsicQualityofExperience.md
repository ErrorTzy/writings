...
**Defense**: Deny 2: a painting is representational, and we can realize there are two different things and can tune our awareness to either the representation or the represented. But when we look at a tree, we cannot tune our awareness to the representation itself. Therefore, naively, the tree is not a representation. 

> But I think the argument from visual experience is saying: It's not that we can't be aware of the representation. We are aware of them, but we mistakenly attribute the features of the representation to the represented object. By reflection, we will realize the features, like how something looks like from a certain angle, are not intrinsic to the tree. Therefore, by reflection, we can distinguish between the representation and the represented. The alternative is to say we see the external object tree directly, but we also project some features to the tree.
