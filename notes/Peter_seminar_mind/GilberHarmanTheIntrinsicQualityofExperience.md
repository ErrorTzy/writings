...
> I think the answer is that, we actually can, but we are not guestured to use the word "disgusting" in this way. For example, we are willing to retract our judgment about red when people point out when there's some abnormality in the lighting condition. However, when we say something is disgusting, we are not disposed to retract even if someone points out we are in some special condition. In other words, we intend some properties to be a projection of our unmistakable mental states, but we intend some properties to be really out there.
